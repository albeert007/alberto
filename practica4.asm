...
contar_letras:
	li	$t0,0 #i
unFor:	bge	$t0,26,dosFor
	sll	$t1,$t0,2
	la	$t2,cuentas_letras
	add	$t3,$t2,$t1
	sw	$zero,($t3)
	add	$t0,$t0,1
	j	unFor
...
dosFor:	li	$t0,0 #i
	la	$t2,texto #texto
inicio:	add	$t3,$t2,$t0 #calculo de la dirección
	lb	$t4,0($t3) #contenido de texto
	beq	$t4,'\0',fin
	move	$t5,$t4	#letra
	blt	$t5,'a',salto	#if
	bgt	$t5,'z',salto	#if
	...
	add	$t7,$t1,$t6	#dirección cuentas_letras[idx]
	lw	$s0,0($t7)
	add	$s1,$s0,1
	sw	$s1,0($t7)	
salto:	add	$t0,$t0,1
